...
export const getRussianCapacity = () => {
  ...
  for (const adjObj of adjectives) {
    for (const rootObj of rootsAndStems) {
      for (const suffixObj of suffixes) {
        const rData = getSlavicData(rootObj.ru);
        const sData = getSlavicData(suffixObj.ru);
        ...
        const adjInflected = inflectSlavicAdjective(adjObj.ru!, gender, 'ru', isPlural ? 'pl' : 'sg');
  ...
  for (const rootObj of rootsAndStems) {
    for (const riverObj of rivers) {
      const rData = getSlavicData(rootObj.ru);
      const rivData = getSlavicData(riverObj.ru);
