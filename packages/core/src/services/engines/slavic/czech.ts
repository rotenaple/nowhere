...
export const getCzechCapacity = () => {
  ...
  for (const adjObj of adjectives) {
    for (const rootObj of rootsAndStems) {
      for (const suffixObj of suffixes) {
        const rData = getSlavicData(rootObj.cs);
        const sData = getSlavicData(suffixObj.cs);
        ...
        const adjInflected = inflectSlavicAdjective(adjObj.cs!, gender, 'cs', isPlural ? 'pl' : 'sg');
  ...
  for (const rootObj of rootsAndStems) {
    for (const riverObj of rivers) {
      const rData = getSlavicData(rootObj.cs);
      const rivData = getSlavicData(riverObj.cs);
...
export const generateCzechPlace = (): GeneratedResult => {
  ...
  if (typeRoll < 0.35) {
    ...
    let shiftedSuffixCs = isDerived ? selectedSuffix?.cs : undefined;
    let suffixSrc = "";
    if (isDerived && selectedSuffix) {
        ...
        if (suffixSrc === 'ov' && Math.random() < 0.08) {
            suffixSrc = 'ová';
            shiftedSuffixCs = [['ová'], 'f'];
        }
    }

    // UPDATED: Get attributes including number
    const { gender, number } = getCompositeAttributes(selectedRootComponent.cs, shiftedSuffixCs);
  ...
  // 3. Adjective + (Root + Suffix) - Explicit Path
    ...
    const suffixInfo = getSlavicData(selectedSuffix.cs!);
    let suffixSrc = suffixInfo.src;
    let shiftedSuffixCs = selectedSuffix.cs;
